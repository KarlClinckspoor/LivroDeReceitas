...
class Ingredient:
    ...
    def __init__(self, ingredient: dict[str, str]):
        ...
        try:
            ...
            print(
                f"Exception {e}. Quantity added isn't a valid number, replaced with 0"
            )
        ...
        if self.quantity == 0:
            raise Warning(
                "Ingredient added without or with invalid quantity! Will appear blank"
            )
        if self.unit == "":
